...
const SUPPORTED_ASPECT_RATIOS = ['1:1', '16:9', '9:16', '2:3', '3:2', '4:5', '5:4'] as const;
const POLL_INTERVAL = 2000; // 2 seconds
const MAX_POLL_TIME = 300000; // 5 minutes

export class MidjourneyHandler extends BaseModelHandler {
  private async pollTask(taskId: string, abortSignal?: AbortSignal): Promise<MidjourneyTaskResponse> {
    ...
    while (true) {
      ...
      const response = await fetchFn(`${this.settings.baseURL}/mj/task/${taskId}/fetch`, {
        method: 'GET',
        headers: {
  ...
  private async upscaleImage(taskId: string, response: MidjourneyTaskResponse, abortSignal?: AbortSignal): Promise<MidjourneyTaskResponse> {
    ...
    const { value: actionResponse } = await postJsonToApi<MidjourneySubmitResponse>({
  ...
  protected async processRequest({
    ...
    const { value: submitResponse } = await postJsonToApi<MidjourneySubmitResponse>({
    ...
    const initialResult = await this.pollTask(submitResponse.result, abortSignal);

    // Automatically upscale the first image
    const finalResult = await this.upscaleImage(submitResponse.result, initialResult, abortSignal);
